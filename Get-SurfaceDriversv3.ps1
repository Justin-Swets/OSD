...
$SystemInfo = @{
    Product        = $ComputerSystem.SystemSkuNumber
...
try {
    ...
    $DriverMap = @()
    foreach ($LandingUrl in $Links) {
        try {
            $Page = Invoke-WebRequest -Uri $LandingUrl -UseBasicParsing -TimeoutSec 10
            ...
            # Filename and Direct Download URL Extraction
            $File = ""
            $DirectDownloadUrl = ""
            
            # Regex to find direct MSI download link from Microsoft's download servers
            if ($Page.Content -match '(https://download\.microsoft\.com/[^"]+?\.msi)') {
                $DirectDownloadUrl = $Matches[1]
                $File = Split-Path -Leaf $DirectDownloadUrl
            }
            
            # Fallback if direct URL wasn't found but filename exists in page content
            if (-not $File -and ($Page.Content -match '\"(Surface[^\"]+?\.msi)\"')) {
                $File = $Matches[1]
            }
            ...
            if ($Name) { 
                $DriverMap += [PSCustomObject]@{ 
                    Model = $Name 
                    URL = if ($DirectDownloadUrl) { $DirectDownloadUrl } else { $LandingUrl } 
                    FileName = $File
                    Date = $Date
                } 
            }
    ...
    if ($FinalSelection) {
        ...
        Write-Host "      Download URL: $($FinalSelection.URL)" -ForegroundColor DarkGray
